...
const getCSS = (dark) => `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sec-wrap { padding:clamp(40px,8vw,80px) clamp(16px,5vw,60px) clamp(60px,10vw,100px); }
  ...
  .stitle { font-family:var(--font-h); font-size:clamp(24px,5vw,56px); font-weight:800; letter-spacing:-1.5px; margin-bottom:16px; line-height:1.08; }
  ...
  .ssub { color:var(--text2); font-size:clamp(14px,2vw,16px); max-width:560px; line-height:1.7; margin-bottom:0; }
  ...
  .hero-sec {
    ...
    position:relative; overflow:hidden; gap:clamp(20px,4vw,40px);
    background:var(--bg);
  }
  ...
  .hero-title { font-family:var(--font-h); font-size:clamp(36px,6vw,82px); font-weight:800; line-height:1.04; letter-spacing:-2px; margin-bottom:20px; animation:fadeUp .6s .2s both; }
  ...
  .hero-sub { font-size:clamp(14px,2vw,16.5px); color:var(--text2); line-height:1.75; max-width:480px; margin-bottom:36px; animation:fadeUp .6s .3s both; }
  .hero-btns { display:flex; gap:16px; flex-wrap:wrap; animation:fadeUp .6s .4s both; }
  .btn-p { padding:13px 30px; background:var(--accent); color:white; border-radius:10px; font-size:15px; font-weight:600; text-decoration:none; transition:all .3s; cursor:pointer; border:none; }
  .btn-p:hover { background:#6d28d9; transform:translateY(-3px); box-shadow:var(--glow); }
  .btn-s { padding:13px 30px; background:transparent; color:var(--text); border:1.5px solid var(--border); border-radius:10px; font-size:15px; font-weight:600; text-decoration:none; transition:all .3s; cursor:pointer; }
  .btn-s:hover { border-color:var(--accent2); color:var(--accent2); transform:translateY(-3px); }
  .hero-stats { display:flex; gap:clamp(16px,3vw,36px); margin-top:clamp(28px,5vw,52px); animation:fadeUp .6s .5s both; flex-wrap:wrap; }
  .stat-num { font-family:var(--font-h); font-size:clamp(22px,3vw,34px); font-weight:800; background:linear-gradient(135deg,var(--accent),var(--accent2)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
  ...
  .hero-3d { position:relative; z-index:2; height:clamp(260px,40vw,520px); display:flex; align-items:center; justify-content:center; animation:fadeUp .8s .3s both; }
  .scene3d { width:clamp(260px,40vw,520px); height:clamp(260px,40vw,520px); position:relative; display:flex; align-items:center; justify-content:center; }

  /* Central core orb */
  .core-orb {
    position:absolute; width:clamp(70px,12vw,130px); height:clamp(70px,12vw,130px); border-radius:50%; z-index:10;
  ...
  .core-label-top { font-size:clamp(6px,1.2vw,10px); font-weight:800; color:rgba(255,255,255,.75); letter-spacing:2px; text-transform:uppercase; }
  .core-label-main { font-family:var(--font-h); font-size:clamp(10px,2vw,17px); font-weight:900; color:#fff; letter-spacing:-0.5px; }
  .core-label-bot { font-size:clamp(6px,1vw,10px); font-weight:700; color:rgba(255,255,255,.6); letter-spacing:1px; text-transform:uppercase; }

  /* Orbit track rings */
  .orbit-track { position:absolute; border-radius:50%; top:50%; left:50%; }
  .track1 { width:clamp(120px,22vw,230px); height:clamp(120px,22vw,230px); margin:calc(clamp(120px,22vw,230px) / -2) 0 0 calc(clamp(120px,22vw,230px) / -2); border:1px solid rgba(124,58,237,.3); }
  .track2 { width:clamp(180px,33vw,340px); height:clamp(180px,33vw,340px); margin:calc(clamp(180px,33vw,340px) / -2) 0 0 calc(clamp(180px,33vw,340px) / -2); border:1px solid rgba(6,182,212,.22); }
  .track3 { width:clamp(240px,44vw,460px); height:clamp(240px,44vw,460px); margin:calc(clamp(240px,44vw,460px) / -2) 0 0 calc(clamp(240px,44vw,460px) / -2); border:1px dashed rgba(245,158,11,.15); }
  ...
  .orbit-bubble {
    position:absolute;
    width:clamp(28px,4.5vw,46px); height:clamp(28px,4.5vw,46px); border-radius:50%;
    display:flex; align-items:center; justify-content:center;
    ...
    top:calc(clamp(28px,4.5vw,46px) / -2);
    animation:iconUpright linear infinite;
  }
  .orbit-bubble img { width:clamp(16px,2.5vw,26px); height:clamp(16px,2.5vw,26px); object-fit:contain; display:block; }
  .orbit-bubble svg { width:clamp(16px,2.5vw,26px); height:clamp(16px,2.5vw,26px); display:block; }
  ...
  .about-inner { margin-top:clamp(28px,5vw,52px); display:grid; grid-template-columns:1.1fr 1fr; gap:clamp(24px,5vw,52px); }
  .about-text p { color:var(--text2); font-size:clamp(14px,1.8vw,15.5px); line-height:1.88; margin-bottom:18px; }
  .hl { color:var(--accent2); font-weight:700; }
  .about-cards { display:grid; grid-template-columns:1fr 1fr; gap:16px; align-content:start; }
  .asc { background:var(--surface); border:1px solid var(--border); border-radius:16px; padding:22px 18px; transition:all .3s; cursor:default; }
  ...
  .skill-tabs { display:flex; gap:8px; margin:clamp(20px,4vw,36px) 0 clamp(16px,3vw,28px); flex-wrap:wrap; }
  .skill-tab { padding:9px 20px; border-radius:8px; font-size:13px; font-weight:600; border:1.5px solid var(--border); background:transparent; color:var(--text2); cursor:pointer; transition:all .3s; }
  .skill-tab.active,.skill-tab:hover { background:var(--accent); border-color:var(--accent); color:white; }
  .skills-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(clamp(100px,18vw,138px),1fr)); gap:clamp(10px,2vw,16px); }
  .skill-card { background:var(--surface); border:1px solid var(--border); border-radius:14px; padding:clamp(14px,3vw,22px) clamp(10px,2vw,14px); text-align:center; transition:all .3s; cursor:default; position:relative; overflow:hidden; }
  ...
  .sk-icon { font-size:clamp(24px,4vw,34px); margin-bottom:10px; display:block; }
  .sk-name { font-size:clamp(11px,1.5vw,12.5px); font-weight:600; color:var(--text2); }
  ...
  .proj-head { display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:clamp(28px,5vw,52px); flex-wrap:wrap; gap:20px; }
  .proj-filters { display:flex; gap:8px; flex-wrap:wrap; }
  .flt-btn { padding:8px 18px; border-radius:8px; font-size:12px; font-weight:600; border:1.5px solid var(--border); background:transparent; color:var(--text2); cursor:pointer; transition:all .3s; text-transform:uppercase; letter-spacing:.5px; }
  .flt-btn.active,.flt-btn:hover { background:var(--accent); border-color:var(--accent); color:white; }
  .projects-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(min(100%,340px),1fr)); gap:clamp(16px,3vw,28px); }
  .project-card { background:var(--surface); border:1px solid var(--border); border-radius:20px; overflow:hidden; transition:all .4s; cursor:default; }
  .project-card:hover { transform:translateY(-8px); border-color:var(--accent); box-shadow:0 20px 60px rgba(0,0,0,.15),var(--glow); }
  .proj-img { width:100%; height:clamp(160px,25vw,210px); overflow:hidden; position:relative; }
  .proj-img-in { width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size:clamp(48px,8vw,78px); transition:transform .4s; }
  ...
  .ov-btn { padding:10px 20px; border-radius:8px; font-size:13px; font-weight:600; text-decoration:none; transition:all .2s; cursor:pointer; }
  ...
  .proj-body { padding:clamp(16px,3vw,24px); }
  .proj-cat { font-size:11px; font-weight:700; letter-spacing:2px; text-transform:uppercase; color:var(--accent2); margin-bottom:8px; }
  .proj-name { font-family:var(--font-h); font-size:clamp(16px,2.5vw,19px); font-weight:700; margin-bottom:10px; color:var(--text); }
  .proj-desc { font-size:clamp(12px,1.8vw,13.5px); color:var(--text2); line-height:1.7; margin-bottom:18px; }
  ...
  .timeline { margin-top:clamp(28px,5vw,52px); position:relative; }
  ...
  .tl-item { padding-left:clamp(40px,8vw,60px); margin-bottom:44px; position:relative; }
  ...
  .tl-card { background:var(--surface); border:1px solid var(--border); border-radius:16px; padding:clamp(16px,3vw,26px); transition:all .3s; }
  ...
  .tl-role { font-family:var(--font-h); font-size:clamp(15px,2.5vw,17px); font-weight:700; color:var(--text); }
  .tl-period { font-size:12px; color:var(--accent2); font-weight:600; padding:4px 12px; background:rgba(6,182,212,.1); border-radius:6px; border:1px solid rgba(6,182,212,.2); white-space:nowrap; }
  .tl-company { font-size:14px; color:var(--accent); font-weight:600; margin-bottom:10px; }
  .tl-desc { font-size:clamp(13px,2vw,14px); color:var(--text2); line-height:1.7; }
...
.edu-course-tag {
  ...
  .cert-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(min(100%,260px),1fr)); gap:clamp(12px,2vw,20px); margin-top:clamp(28px,5vw,52px); }
  .cert-card { background:var(--surface); border:1px solid var(--border); border-radius:16px; padding:clamp(16px,3vw,24px); display:flex; gap:16px; align-items:flex-start; transition:all .3s; cursor:default; }
  .cert-card:hover { border-color:var(--accent); transform:translateY(-4px); box-shadow:0 10px 25px rgba(124,58,237,0.25); }
  .cert-icon { font-size:32px; flex-shrink:0; }
  .cert-name { font-family:var(--font-h); font-size:clamp(13px,2vw,15px); font-weight:700; color:var(--text); margin-bottom:4px; }
  .cert-org { font-size:13px; color:var(--accent); font-weight:600; margin-bottom:3px; }
  .cert-year { font-size:12px; color:var(--text3); }
  .cert-highlight { display:inline-block; font-size:10px; background:#7c3aed; color:#fff; padding:3px 6px; border-radius:5px; margin:4px 0; }
  .cert-logo { width:28px; height:28px; object-fit:contain; margin-bottom:4px; }

  /* ── CONTACT ── */
  .contact-sec { background:var(--bg2); }
  .contact-grid { display:grid; grid-template-columns:1fr 1.4fr; gap:clamp(24px,5vw,56px); margin-top:clamp(28px,5vw,52px); align-items:start; }
  .ct-info { background:var(--surface); border:1px solid var(--border); border-radius:20px; padding:clamp(20px,4vw,36px); }
  .ct-info h3 { font-family:var(--font-h); font-size:clamp(17px,3vw,21px); font-weight:700; margin-bottom:8px; color:var(--text); }
  ...
  .ct-link { display:flex; align-items:center; gap:14px; text-decoration:none; color:var(--text); padding:13px 15px; background:var(--surface2); border-radius:12px; border:1px solid var(--border); transition:all .3s; font-size:14px; font-weight:500; cursor:pointer; }
  ...
  .ct-form { background:var(--surface); border:1px solid var(--border); border-radius:20px; padding:clamp(20px,4vw,36px); }
  .form-title { font-family:var(--font-h); font-size:clamp(17px,3vw,21px); font-weight:700; margin-bottom:26px; color:var(--text); }
  .fg { margin-bottom:18px; }
  .frow { display:grid; grid-template-columns:1fr 1fr; gap:16px; }
  .fg label { display:block; font-size:13px; font-weight:600; color:var(--text2); margin-bottom:7px; }
  .fg input,.fg textarea,.fg select { width:100%; background:var(--surface2); border:1.5px solid var(--border); border-radius:10px; color:var(--text); font-family:var(--font-b); font-size:14px; padding:11px 15px; transition:all .3s; outline:none; -webkit-appearance:none; }
  ...
  .form-btn { width:100%; padding:13px; background:linear-gradient(135deg,var(--accent),var(--accent2)); color:white; border:none; border-radius:10px; font-family:var(--font-b); font-size:15px; font-weight:700; cursor:pointer; transition:all .3s; display:flex; align-items:center; justify-content:center; gap:8px; }
  ...
  .availability { font-size:13px; color:#22c55e; margin-top:6px; }
  ...
  .res-bar { height:60px; background:var(--nav-bg); backdrop-filter:blur(20px); border-bottom:1px solid var(--border); display:flex; align-items:center; justify-content:space-between; padding:0 clamp(16px,4vw,32px); flex-shrink:0; gap:12px; }
  .res-bar-title { font-family:var(--font-h); font-size:clamp(13px,2vw,16px); font-weight:700; background:linear-gradient(135deg,var(--accent),var(--accent2)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .res-close { padding:8px 20px; background:transparent; border:1.5px solid var(--border); border-radius:8px; color:var(--text2); font-size:13px; font-weight:600; cursor:pointer; transition:all .3s; white-space:nowrap; flex-shrink:0; }
  .res-close:hover { border-color:var(--accent); color:var(--accent); }
  .res-body { flex:1; overflow:auto; display:flex; align-items:flex-start; justify-content:center; padding:clamp(12px,3vw,32px); background:var(--bg2); }
  .res-page { background:white; width:min(800px, 100%); border-radius:8px; box-shadow:0 20px 80px rgba(0,0,0,.3); overflow:hidden; font-family:'Georgia',serif; color:#111; }
  .rp-hdr { background:#0a0a1a; color:#fff; padding:clamp(20px,4vw,36px) clamp(20px,5vw,44px) clamp(16px,3vw,28px); }
  .rp-name { font-size:clamp(22px,5vw,32px); font-weight:900; letter-spacing:-1px; margin-bottom:4px; font-family:'Syne',sans-serif; }
  .rp-role { font-size:clamp(11px,1.8vw,13px); color:rgba(255,255,255,.6); margin-bottom:18px; letter-spacing:.5px; }
  .rp-contacts { display:flex; gap:clamp(8px,2vw,18px); flex-wrap:wrap; }
  .rp-ct { font-size:clamp(10px,1.5vw,12px); color:rgba(255,255,255,.8); display:flex; align-items:center; gap:5px; }
  .rp-body { display:grid; grid-template-columns:1fr 1.6fr; }
  .rp-left { background:#f8f8f8; padding:clamp(16px,3vw,28px) clamp(14px,3vw,24px); }
  .rp-right { background:#fff; padding:clamp(16px,3vw,28px) clamp(14px,3vw,24px); }
  ...
  .footer-main { display:grid; grid-template-columns:2fr 1fr 1fr 1.2fr; gap:clamp(20px,4vw,40px); padding:clamp(32px,6vw,52px) clamp(16px,5vw,60px) clamp(24px,4vw,40px); }
  .footer-brand { }
  ...
  .f-soc { width:36px; height:36px; border-radius:9px; background:var(--surface); border:1px solid var(--border); display:flex; align-items:center; justify-content:center; font-size:16px; text-decoration:none; transition:all .3s; cursor:pointer; }
  ...
  .footer-link { color:var(--text2); text-decoration:none; font-size:14px; transition:color .2s; cursor:pointer; display:flex; align-items:center; gap:6px; background:none; border:none; font-family:var(--font-b); }
  ...
  .fci-val { font-size:13px; color:var(--text2); word-break:break-word; }
  .footer-bottom { border-top:1px solid var(--border); padding:clamp(14px,3vw,20px) clamp(16px,5vw,60px); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; }
  ...
  .footer-availability { font-size:12px; color:#22c55e; margin-top:6px; }

  /* Project image */
  .proj-image { width:100%; height:100%; object-fit:contain; }
  ...
  /* ═══════════════════════════════════════════════════
     RESPONSIVE BREAKPOINTS — every major device size
  ═══════════════════════════════════════════════════ */

  /* ── Large desktop (1280px+) — already default ── */

  /* ── Medium desktop / laptop (960px–1279px) ── */
  ...
  @media (max-width: 959px) {
    ...
    .hero-title { font-size: clamp(36px, 7vw, 56px); letter-spacing: -1px; }
    .hero-3d { height: clamp(280px, 50vw, 400px); }
    .scene3d { width: clamp(280px, 50vw, 400px); height: clamp(280px, 50vw, 400px); }
  ...
  @media (max-width: 767px) {
    .hero-sec { padding: 40px 16px 52px; }
    .hero-title { font-size: clamp(32px, 8vw, 52px); }
    .hero-sub { font-size: 14px; }
    .hero-btns { gap: 12px; }
    .btn-p, .btn-s { padding: 11px 22px; font-size: 14px; }

    .hero-3d { height: clamp(260px, 55vw, 340px); }
    .scene3d { width: clamp(260px, 55vw, 340px); height: clamp(260px, 55vw, 340px); }
    ...
    .flt-btn { padding: 6px 14px; font-size: 11px; }
  ...
  @media (max-width: 599px) {
    .sec-wrap { padding: 36px 16px 56px; }
    .stitle { font-size: clamp(22px, 7vw, 36px); letter-spacing: -0.5px; }

    .hero-sec { padding: 36px 14px 48px; }
    ...
    .scene3d { width: 260px; height: 260px; }

    .about-cards { grid-template-columns: 1fr 1fr; }
    .asc-num { font-size: 22px; }

    .skills-grid { grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 10px; }
    .sk-icon { font-size: 26px; }
    ...
    .rp-hdr { padding: 20px 16px 16px; }
    .rp-name { font-size: 20px; }
    .rp-contacts { gap: 6px; }

    .proj-filters { width: 100%; }
    .flt-btn { flex: 1; text-align: center; }

    .tl-tags { gap: 5px; }
    .tl-tag { font-size: 11px; padding: 3px 9px; }
  }

  /* ── Mobile small (360px–479px) ── */
  @media (max-width: 479px) {
    .sec-wrap { padding: 28px 12px 44px; }
    .hero-sec { padding: 28px 12px 40px; }
    .hero-title { font-size: clamp(26px, 9vw, 40px); }
    .hero-btns { flex-direction: column; gap: 10px; }
    .btn-p, .btn-s { width: 100%; text-align: center; justify-content: center; display: flex; align-items: center; }

    .hero-3d { height: 220px; }
    .scene3d { width: 220px; height: 220px; }
    .core-orb { width: 56px; height: 56px; }
    .core-label-main { font-size: 9px; }
    .core-label-top, .core-label-bot { font-size: 5px; }

    .about-cards { grid-template-columns: 1fr 1fr; gap: 10px; }
    .asc { padding: 16px 12px; }
    .asc-icon { font-size: 20px; }

    .skill-tabs { gap: 5px; }
    .skill-tab { padding: 7px 11px; font-size: 11px; }
    ...
    .ct-info, .ct-form { padding: 18px; }
    .ct-links { gap: 10px; }
    .ct-link { padding: 11px 12px; gap: 10px; font-size: 13px; }
    ...
    .footer-main { grid-template-columns: 1fr; padding: 24px 12px 20px; }
    .footer-bottom { padding: 14px 12px; flex-direction: column; align-items: flex-start; }
    .footer-badges { gap: 6px; }
    .footer-badge { font-size: 10px; padding: 3px 10px; }
  }
  ...
  @media (max-width: 359px) {
    ...
    .scene3d { width: 180px; height: 180px; }
    .core-orb { width: 44px; height: 44px; }
  ...
  @media (min-width: 1440px) {
    .sec-wrap { padding: 90px 80px 110px; }
    .hero-sec { padding: 70px 80px; }
    nav { padding: 0 80px; }
    .footer-main { padding: 60px 80px 48px; }
    .footer-bottom { padding: 22px 80px; }
  }

  /* ── Ultra wide (1920px+) ── */
  @media (min-width: 1920px) {
    .sec-wrap { max-width: 1600px; margin-left: auto; margin-right: auto; }
    .hero-sec { max-width: 1600px; margin-left: auto; margin-right: auto; }
    nav { max-width: 100%; }
    .footer-main { max-width: 1600px; margin-left: auto; margin-right: auto; }
  }

  /* ── Landscape phone ── */
  @media (max-height: 500px) and (orientation: landscape) {
    .hero-sec { min-height: auto; padding: 20px 16px; }
    .hero-3d { height: 200px; }
    .scene3d { width: 200px; height: 200px; }
    nav { height: 54px; }
    .mob-drawer { top: 54px; }
  ...
  /* ── Print ── */
  @media print {
    .cursor, .cursor-ring, nav, .mob-drawer, .hamburger, .nav-resume, .theme-toggle, .hero-3d, footer { display: none !important; }
    main { padding-top: 0 !important; }
    .hero-sec { grid-template-columns: 1fr; min-height: auto; }
    body { cursor: auto; }
  }
`;
...
const particles = Array.from({ length: 16 }, (_, i) => ({
  ...
  delay: Math.random() * 5, dur: 4 + Math.random() * 6, dx: (Math.random() - .5) * 80
}));
      ...
      <div className="hero-sec">
        ...
        <div className="hero-content">
          ...
          <div className="hero-btns">
            <button className="btn-p" onClick={() => navigate("projects")}>🚀 View Projects</button>
            <button className="btn-s" onClick={() => navigate("contact")}>💼 Hire Me</button>
          </div>
        ...
        <div className="hero-3d">
          <div className="scene3d">
            ...
            {orbitData.map(({ r, dur, del, rev, svg }, i) => (
            ...
            {particles.map(p => (
              <div key={p.id} className="pt" style={{
                ...
                "--dx": `${p.dx}px`,
                animation: `ptFloat ${p.dur}s ${p.delay}s linear infinite`
              }} />
            ))}
          </div>
        </div>
      ...
      <div className="sec-wrap about-sec" style={{ borderTop: "1px solid var(--border)" }}>
        ...
        <div className="about-inner">
          <div className="about-text">
            <p>Hi! I'm <span className="hl">Vure Maneesh</span>, a Computer Science graduate and Backend-focused Software Developer with strong expertise in Java, Spring Boot, and REST API development.</p>
            <p>I specialize in building scalable backend systems with clean architecture, focusing on efficient API design, secure data handling, and reliable application performance using technologies like Spring Boot, JPA/Hibernate, and MySQL.</p>
            <p>Currently working as a <span className="hl">Software Developer Intern at Infosys</span>, where I contribute to backend development, work on RESTful services, and collaborate within Agile Scrum teams to build real-world application modules.</p>
            <p>I also have experience building responsive user interfaces using HTML and CSS, and leverage modern AI-assisted tools to accelerate frontend development and improve productivity.</p>
            <p>I am passionate about <span className="hl">designing scalable and production-ready systems</span>, continuously improving my problem-solving skills, and building software that delivers real-world impact.</p>
  ...
  return (
    <div>
      ...
      <div className="sec-wrap exp-sec" style={{ borderTop: "1px solid var(--border)" }}>
        ...
        <p className="ssub">Hands-on experience through my internship at Infosys, contributing to backend development using Java, Spring Boot, and REST APIs.</p>
            ...
            <div className="proj-img">
              <div className="proj-img-in" style={{ background: p.bg }}>
                ...
                <div className="proj-overlay">
                  {p.demo && (
                    <a href={p.demo} target="_blank" rel="noreferrer" className="ov-btn ov-live">🔗 Live Demo</a>
                  )}
                  <a href={p.link} target="_blank" rel="noreferrer" className="ov-btn ov-code">⌨️ Code</a>
                </div>
              </div>
            </div>
...
function EducationView() {
  return (
    <div className="sec-wrap edu-sec">
      ...
      <p className="ssub">Academic foundation in Computer Science and Engineering with focus on Java, Data Structures, DBMS, and Object-Oriented Programming.</p>
...
function ContactView() {
  ...
  const sub = (e) => {
    ...
    window.location.href = `mailto:${receiver}?subject=${mailSubject}&body=${mailBody}`;
  };
  ...
  return (
    <div className="sec-wrap contact-sec">
      ...
      <p className="ssub">Open to Software Developer roles, internships, and backend development opportunities. Let's build scalable and impactful solutions together.</p>
      <p className="availability">🟢 Available for immediate opportunities</p>

      <div className="contact-grid">
        <div className="ct-info">
          <h3>Say Hello 👋</h3>
          <p>I'm currently a Software Developer Intern at Infosys, specializing in backend development using Java and Spring Boot. I enjoy building scalable applications and actively seeking opportunities where I can contribute to real-world software systems.</p>
          <div className="ct-links">
            ...
            <div className="ct-link">
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "var(--text3)", marginTop: 2 }}>Hyderabad, Telangana, India</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="ct-form">
          ...
          {sent ? (
            <div className="form-ok">✅ Message sent! I'll get back to you within 24 hours.</div>
          ) : (
            <form onSubmit={sub}>
              <div className="frow">
                <div className="fg">
              ...
              <div className="fg">
                <label>Message</label>
                <textarea placeholder="Share details about the opportunity, project, or idea..." required value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} />
              </div>
    ...
    <footer>
      <div className="footer-main">
        <div className="footer-brand">
          <p className="f-tagline">Backend-focused Software Developer specializing in Java, Spring Boot, and REST API development. Passionate about building scalable systems and real-world applications.</p>
          <div className="f-social-row">
            {[
              ...
              { i: "📄", h: "https://drive.google.com/file/d/1Wsdon8lVg4QhTaanjVqTWffHaxKEkf1d/view?usp=sharing" }
            ].map((s, idx) => (
              <a key={idx} href={s.h} target="_blank" rel="noreferrer" className="f-soc">{s.i}</a>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="footer-links">
            {NAV_ITEMS.map(n => (
              <button key={n.id} className="footer-link" onClick={() => navigate(n.id)}>{n.label}</button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="footer-links">
            {["Java · Spring Boot", "REST APIs · Hibernate", "MySQL · PostgreSQL", "HTML · CSS · JavaScript", "Git · GitHub", "OOP · SDLC"].map(t => (
      ...
      <div className="footer-bottom">
        <div className="footer-copy">© {year} Vure Maneesh · Built with React</div>
...
export default function Portfolio() {
  ...
  useEffect(() => {
    const h = () => { if (window.innerWidth >= 960) setMobileOpen(false); };
    ...
    <div className="page-shell">
      ...
      <nav className={navScrolled ? "scrolled" : ""}>
        ...
        <div className="nav-right">
          ...
          <button className={`hamburger${mobileOpen ? " open" : ""}`} onClick={() => setMobileOpen(o => !o)} aria-label="Toggle menu">
            <span /><span /><span />
          </button>
        </div>
      </nav>
